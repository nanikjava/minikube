...
then
    ...
    missing="$($GO run ${BDIR}/boilerplate.go --rootdir . --boilerplate-dir ${BDIR} | egrep -v 'site/themes/docsy/userguide/deploy.sh|assets.go|translations.go|/site/themes/|/site/node_modules|\./out|/hugo/' || true)"
    echo "..... ${missing} ........ "
    if [[ -n "${missing}" ]]; then
